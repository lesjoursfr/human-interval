/**
 * Original code by @blakeembrey
 * https://github.com/blakeembrey/node-numbered
 */
const NUMBER_MAP = Object.freeze<{ [key: number | string]: string }>({
  ".": "point",
  "-": "negative",
  0: "zero",
  1: "one",
...
const CARDINAL_MAP = Object.freeze<{ [key: number]: string }>({
  2: "hundred",
  3: "thousand",
  6: "million",
  9: "billion",
...
(Object.keys(NUMBER_MAP) as unknown as Array<keyof typeof NUMBER_MAP>).forEach(function (num) {
  WORD_MAP[NUMBER_MAP[num]] = isNaN(+num) ? num : +num;
});
(Object.keys(CARDINAL_MAP) as unknown as Array<keyof typeof CARDINAL_MAP>).forEach(function (num) {
 ...
 * @return {number}
 */
function totalStack(stack: Array<number>, largest: number): number {
 ...
 */
function numbered(num: string | number): string | number {
 ...
 * @return {string}
 */
numbered.stringify = function (value: number): string {
 ...
 * @return {number}
 */
numbered.parse = function (num: string): number {
  let modifier = 1;
  let largest = 0;
  ...
  const total = num
    ...
    .reduceRight<number>(function (memo, num) {
      const interval = intervals(num as number);
      ...
      largest = num as number;
      largestInterval = intervals(largest);

      return (memo + (num as number)) * Math.pow(10, zeros);
    }, 0);
